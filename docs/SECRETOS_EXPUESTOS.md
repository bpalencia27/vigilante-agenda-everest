...
1. **Siempre-Encendida por política del dueño (desde v17.58.2, 29-ago):** este punto describía la política Default-Off original (R1.8), que YA NO RIGE y se sincroniza aquí en v18.0.134 (refutación R1 de la auditoría del 03-sep-2026, ítem 7 de su plan). El script viene de fábrica con `reporte: true` y `uxTelemetria: true` — decisión explícita y documentada del dueño (el precio de usar el script gratis), forzada además en el arranque: ni una configuración guardada con `false` ni una edición manual de `vgl_cfg` la apagan. La envolvente de seguridad no es un interruptor sino la construcción: lo que sale del equipo son métricas agregadas anónimas por diseño (cero PHI, ver `docs/TELEMETRIA.md`), y la bitácora local pasa por `sanitizePII` campo a campo y nunca se transmite.
