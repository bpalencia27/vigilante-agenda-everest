...
| 22 | cerrada; sitios hermanos: `pesoFaltaParaEstadio` buscaba «peso» exacto → con 15 kg el plan de ERC decía «no hay ningún examen que vigilar» (la frase que v17.6.51 llamó mentira); la hoja de hechos llevaba «peso 15 kg» sin marca | v18.0.106 (#286, #287) |
| 24b | cerrada (M158 muerde); prueba hueca: la huella `vglTocada` solo la escribe el listener `change` y ninguna prueba lo disparaba | v18.0.106 (#292) |
| 27 | cerrada; prueba hueca: «en la raíz se conserva la coincidencia laxa» (`Agenda_v2_<hoy>.xls`) sin fijar | v18.0.106 (#293) |
| 30 | cerrada; prueba hueca: la conductual miraba el canal (`console.warn`), no el efecto — el defecto con el aviso conservado pasaba | v18.0.106 (#294) |
| 33a | cerrada; sitio hermano: «Guardar»/«Borrar» credenciales de Athenea repintaban con `renderSettings()` y vaciaban el borrador; prueba hueca: «Guardar y salir» sin continuación pasaba | v18.0.106 (#288, #290) |
| 33b | cerrada para `!bar`; prueba hueca: un mutante en el que solo el `catch` recursaba pasaba (en el navegador, «Maximum call stack» si `querySelector` lanza); incompleta: las salidas de emergencia no abrían la hoja pedida ni vaciaban el borrador | v18.0.106 (#289, #289b) |
| 34 | cerrada; sitio hermano: `isAgendamientoPendiente` leía `vgl_proc_today` una vez por tarjeta | v18.0.106 (#291) |
| — | residuo: con el almacén lleno, «Guardar» cantaba «guardados» sobre un `vgl_cfg` sin persistir | v18.0.106 (#295) |
