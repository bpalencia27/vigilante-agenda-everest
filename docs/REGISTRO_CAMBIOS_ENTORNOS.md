...
| 2026-09-09 | claude/merge-completion-formalize-26b88c | Claude Code (worktree aislado) | Sonnet 5 | vigilante_agenda.user.js, package.json, tests/suite_75_disco.js, tests/suite_101_permisos.js, tests/INFORME_MUTACIONES.md | Solicitud F, paso F3: "Permisos por médico (administración)" (`grpPermisos`) expone y deja tocar los permisos de TODOS los médicos del equipo — ahora exige además `mtrEsDesarrollador()` (F2), no solo `accesoCap("toggles_funcionalidades")` (cualquier COMPLETO); "Funcionalidades por médico" (`grpToggles`, personal, no expone a terceros) sigue igual | `node tests/runner.js` → 3796 pasan EXIT=0 · mutación en `grpPermisos` verificada (roja 15/1, restaurada 16/0) · `compat-check` → COMPATIBLE (18.14.2) | pendiente revisión humana |
| 2026-09-09 | claude/merge-completion-formalize-26b88c | Claude Code (worktree aislado) | Sonnet 5 | (ninguno — auditoría sin cambios) | Solicitud F, paso F4 (auditoría de duplicados): grep de nombres de función (`^\s*function X`) sin colisiones reales a nivel de módulo (los 2 casos con nombre repetido, `progreso`/`makeYielder`, viven dentro de un worker embebido como texto, ámbito global distinto — no son colisión); grep de selectores CSS repetidos: los que aparecían 2-7 veces (`.vgl-agm-*`) resultaron ser el MISMO nombre de clase reutilizado a propósito en selectores compuestos que agrupan varios `#vgl-XXX-modal` distintos (patrón de consolidación YA aplicado, no duplicación); revisado un solapamiento aparente (`mtrTerminosLsLeer/Guardar` vs `obsGmLeer/obsGmGuardar`) — no es duplicado: el primero es localStorage-only A PROPÓSITO (residencia doble v18.8.3, GM + localStorage por separado); no se encontró ninguna consolidación segura y de bajo riesgo que valiera la pena aplicar en esta pasada — casilla vacía antes que dato inventado | sin prueba — no se tocó código; accidente y corrección documentados aparte (`docs/MAPA_v14.md`/`docs/DEUDA_v14.md` regenerados por `node tools/inventario.js` y borrados por error, restaurados con `git checkout --`) | pendiente revisión humana |
| 2026-09-09 | claude/merge-completion-formalize-26b88c | Claude Code (worktree aislado) | Sonnet 5 | vigilante_agenda.user.js, package.json, tests/suite_75_disco.js, tests/suite_88_rcv_pendientes.js, tests/suite_102_widget_rcv.js, tests/INFORME_MUTACIONES.md | Solicitud F, paso F5: el panel «Próximos exámenes RCV» ya no se auto-abre al entrar a la historia — el estado de fábrica pasó de expandido a MINIMIZADO (`_rcvpMinimizado` nace en `true`); solo una pastilla sin datos de paciente asoma cuando hay programa RCV identificado, y el médico decide cuándo verla/cerrarla. El chequeo de minimizado se movió a después de confirmar que el paciente tiene programa RCV, para que la pastilla nunca aparezca sin nada real que mostrar. Nueva `_rcvpExpandirParaTest()` para que las suites de contenido (88, 102) simulen el clic de apertura una sola vez por instancia | `node tests/runner.js` → 3797 pasan EXIT=0 · mutación en `_rcvpMinimizado` verificada (roja 24/1, restaurada 25/0) · `compat-check` → COMPATIBLE (18.14.3) | pendiente revisión humana |
| 2026-09-09 | claude/merge-completion-formalize-26b88c | Claude Code (worktree aislado) | Sonnet 5 | vigilante_agenda.user.js, package.json, tests/suite_75_disco.js, tests/suite_114_fatiga_visual.js (nueva), tests/INFORME_MUTACIONES.md | Solicitud F, paso F6: el agrupamiento y el tope de 4 toasts simultáneos YA EXISTÍAN (v17.6.9/v17.11.0); se añadió lo que faltaba — preferencia "avisos rutinarios silenciados" (`S.avisosRutinariosOff`, calla AZUL/VERDE/FUCSIA; ROJO/MORADO/AMBAR JAMÁS se silencian, invariante de seguridad) y una central de notificaciones mínima (`_avisoHistorial`, anillo de 30, SOLO color+hora, cero PHI) con vista de solo lectura en Ajustes | `node tests/runner.js` → 3805 pasan EXIT=0 · mutación en `_avisoRutinarioSilenciado` verificada (roja 6/2, restaurada 8/0) · `compat-check` → COMPATIBLE (18.14.4) | pendiente revisión humana |
