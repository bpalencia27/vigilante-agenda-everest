...
(function () {
  ...
  const VERSION = "5.0.0"; // fuente única de la versión (título + diagnóstico)
  ...
  const DEFAULTS = {
    ...
    autoRefrescarPym: true,   // si el archivo del día cambia en SharePoint, recargarlo solo
  };
  ...
  function schedulePymRetry() {
    ...
    state.spTries++;
    setTimeout(async () => { if (state.pymFile) return; if (loadPymFromCache()) return; const ok = await autoPymFromSharepoint(true); if (!ok) schedulePymRetry(); }, wait);
  }
  ...
  function buildOverlay() {
    ...
    style.textContent = `
      ...
      .vgl-fld .hint{display:block;font-size:10.5px;color:var(--fg3);font-weight:400;margin-top:2px;max-width:250px;line-height:1.35}
      ...
      .vgl-bar .col{width:100%;display:flex;flex-direction:column-reverse;gap:2px;height:52px;justify-content:flex-start}
      .vgl-bar .seg{width:100%;border-radius:3px;min-height:2px}
      .vgl-bar .lb{font-size:9.5px;color:var(--fg3);font-weight:600}
      ...
      .vgl-kpi .n{font-size:22px;font-weight:700;font-variant-numeric:tabular-nums;line-height:1}
      .vgl-kpi .l{font-size:10px;color:var(--fg3);margin-top:4px;font-weight:600;letter-spacing:.2px}
  ...
  function renderResumen() {
    ...
    const bars = dias.map((d) => {
      ...
      const seg = (n, c) => (n ? `<div class="seg" style="height:${Math.max(2, Math.round((n / max) * 52))}px;background:${c}" title="${n}"></div>` : "");
      return `<div class="vgl-bar"><div class="col">${seg(d.atiempo, COLORS.VERDE)}${seg(d.inasistencia, COLORS.AMBAR)}${seg(d.fraude, COLORS.ROJO)}</div><span class="lb">${dow[t.getDay()]} ${t.getDate()}</span></div>`;
    }).join("");
    el.sheet.innerHTML = sheetHeader("Resumen del turno") + `
      <div class="vgl-kpis">
        <div class="vgl-kpi"><div class="n" style="color:${COLORS.ROJO}">${hoy.fraude || 0}</div><div class="l">FRAUDES</div></div>
        <div class="vgl-kpi"><div class="n" style="color:${COLORS.AMBAR}">${hoy.inasistencia || 0}</div><div class="l">INASISTENCIAS</div></div>
        <div class="vgl-kpi"><div class="n" style="color:${COLORS.VERDE}">${hoy.atiempo || 0}</div><div class="l">A TIEMPO</div></div>
      </div>
      ...
      <div class="vgl-grp">
        <div class="vgl-fld"><label>Eventos registrados hoy<span class="hint">Cambios de estado y alertas, con hora exacta.</span></label><b style="font-variant-numeric:tabular-nums">${evs.length}</b></div>
        <div class="vgl-fld"><label>Reporte de auditoría<span class="hint">Archivo .csv que se abre en Excel. No sale del computador.</span></label><button class="vgl-btn primary" id="vgl-exp">Descargar</button></div>
        <div class="vgl-fld"><label>Copiar resumen<span class="hint">Para pegarlo en un correo o en el acta del turno.</span></label><button class="vgl-btn" id="vgl-copy">Copiar</button></div>
      </div>
      <div class="vgl-grp">
        <div class="vgl-fld"><label>Archivo PyM en uso<span class="hint">${escapeHtml(state.pymFile || "ninguno")}</span></label><b style="font-variant-numeric:tabular-nums">${state.pym.size}</b></div>
        <div class="vgl-fld"><label>Diagnóstico técnico<span class="hint">Sin datos de pacientes. Úsalo si algo deja de funcionar.</span></label><button class="vgl-btn" id="vgl-diag">Diag</button></div>
      </div>`;
    wireClose();
  ...
  function renderSettings() {
    ...
    el.sheet.innerHTML = sheetHeader("Ajustes") + `
      <div class="vgl-grp">
        <div class="vgl-fld"><label>Tolerancia<span class="hint">Minutos de gracia antes de marcar inasistencia. El aviso morado sale 1 min antes.</span></label><input type="number" id="c-tol" step="0.5" min="0.5" max="60" value="${S.tolerancia}"></div>
        <div class="vgl-fld"><label>Refresco<span class="hint">Segundos entre lecturas de la agenda.</span></label><input type="number" id="c-ref" step="1" min="2" max="120" value="${S.refresco}"></div>
        <div class="vgl-fld"><label>Tema<span class="hint">"Automático" sigue el modo claro/oscuro de Windows.</span></label>
      ...
      <div class="vgl-grp">
        <div class="vgl-fld"><label>Sonido<span class="hint">Un tono distinto por color de alerta.</span></label>${sw("c-snd", S.sonido)}</div>
        <div class="vgl-fld"><label>Volumen</label><input type="range" id="c-vol" min="2" max="60" value="${Math.round(S.volumen * 100)}"></div>
        <div class="vgl-fld"><label>Insistir en el fraude<span class="hint">El rojo repite el sonido cada 9 s hasta que lo reconozcas.</span></label>${sw("c-ins", S.insistir)}</div>
        <div class="vgl-fld"><label>Cartel grande<span class="hint">Aviso a pantalla completa dentro de Everest cuando hay fraude.</span></label>${sw("c-car", S.cartel)}</div>
        <div class="vgl-fld"><label>Pestaña parpadeando<span class="hint">Título e ícono del navegador titilan hasta que vuelvas.</span></label>${sw("c-par", S.parpadeo)}</div>
        <div class="vgl-fld"><label>Ventana emergente<span class="hint">Aparece en la barra de tareas de Windows. Hay que permitir ventanas emergentes al sitio.</span></label>${sw("c-pop", S.popup)}</div>
        <div class="vgl-fld"><label>Probar los avisos<span class="hint">Dispara una alerta de prueba de cada tipo.</span></label><button class="vgl-btn" id="c-test">Probar</button></div>
      </div>
      <div class="vgl-grp">
        <div class="vgl-fld"><label>PyM que se ocultan<span class="hint">Separadas por coma. VIH nunca se oculta, aunque lo escribas.</span></label><input type="text" id="c-exc" value="${escapeHtml(S.excluir)}"></div>
        <div class="vgl-fld"><label>Recordar cargar el PyM<span class="hint">Si a esta hora no hay PyM, avisa. Vacío = nunca.</span></label><input type="time" id="c-rec" value="${escapeHtml(S.recordatorio)}"></div>
        <div class="vgl-fld"><label>Recargar si cambia<span class="hint">Si vuelven a subir el archivo del día corregido, lo toma solo.</span></label>${sw("c-auto", S.autoRefrescarPym)}</div>
        <div class="vgl-fld"><label>Carpeta de SharePoint<span class="hint">Pega aquí el enlace de la carpeta ("Copiar vínculo") y se configura sola.</span></label><input type="text" id="c-sp" placeholder="(la de fábrica)" value="${escapeHtml(S.spFolder)}"></div>
      </div>
      <div class="vgl-grp">
        <div class="vgl-fld"><label>Restablecer todo<span class="hint">Vuelve a los valores de fábrica. No borra el historial.</span></label><button class="vgl-btn off" id="c-reset">Restablecer</button></div>
      </div>`;
    wireClose();
  ...
  function renderStats(list) {
    ...
    el.stats.innerHTML =
      `<span class="vgl-stat"><span class="d" style="background:${COLORS.VERDE}"></span>En sala <b>${ensala}</b></span>` +
      `<span class="vgl-stat"><span class="d" style="background:${COLORS.AMBAR}"></span>Sin presentarse <b>${pend}</b></span>` +
      `<span class="vgl-stat"><span class="d" style="background:var(--fg3)"></span>Atendidas <b>${atend}</b></span>` +
  ...
  function downloadDiagnostic() {
    ...
    out.push("===== DIAGNÓSTICO — VIGILANTE v" + VERSION + " =====", "Fecha: " + new Date().toISOString(), "Origen: " + (cloneDoc() ? "CLON" : "página"), "URL: " + location.href, "Título: " + document.title,
      ...
      "Modificado en SharePoint: " + (state.pymMTime || "n/a"), "Carpeta: " + CONFIG.SP.folder,
  ...
  function boot() {
    ...
    if (!loadPymFromCache()) { Promise.resolve(autoPymFromSharepoint(true)).then((ok) => { if (!ok) schedulePymRetry(); }); }
    // Cada 10 min: si cambió el día, si aún falta el PyM, o si volvieron a subir el archivo corregido.
    setInterval(() => {
      if (!cacheIsToday() || !state.pymFile) { if (!loadPymFromCache()) autoPymFromSharepoint(true); }
      else refreshPymIfChanged();
    }, 10 * 60 * 1000);
