...
async def run_victory_tests():
    print("=" * 70)
    print("RE-AUDIT VICTORY AUDITOR INDEPENDENT LIVE TEST EXECUTION")
    print("=" * 70)
    
    results = {}
    
    # Step 1: Start AtheneaService
    ...
    try:
        ...
        # Also test via TestClient endpoint call
        print("\n[AC2 HTTP ENDPOINT TEST] Calling GET /api/buscar_laboratorios?documento=1017214911 via FastAPI TestClient...")
        t0_ac2_http = time.perf_counter()
        resp_ac2 = client.get("/api/buscar_laboratorios?documento=1017214911")
        t1_ac2_http = time.perf_counter()
        duration_ac2_http = t1_ac2_http - t0_ac2_http
        print(f"AC2 HTTP Status Code: {resp_ac2.status_code}")
        print(f"AC2 HTTP Response JSON: {resp_ac2.json()}")
        print(f"AC2 HTTP Execution Time: {duration_ac2_http:.3f} seconds")
        
        ac2_http_pass = (resp_ac2.status_code == 200) and (resp_ac2.json() == {"idSolicitud": 374116}) and (duration_ac2_http < 10.0)
        results["AC2_HTTP"] = {
            "pass": ac2_http_pass,
            "status_code": resp_ac2.status_code,
            "response": resp_ac2.json(),
            "duration_seconds": round(duration_ac2_http, 3)
        }
    ...
    print("\n" + "=" * 70)
    print("SUMMARY OF RE-AUDIT INDEPENDENT EXECUTION")
