...
## TAREA E — Comprobaciones fantasma: suites 05 y 07 no cuentan sus casoAsync

Hallazgo CONFIRMADO por la revisión adversarial del 11-08-2026: `suite_05_api_everest.js`
declara `pruebas(t, api, env, cargar)` SIN `async` y dispara 8 `t.casoAsync(...)` sin
`await` (líneas 5, 31, 41, 73, 86, 104, 123, 137); `suite_07_excel_parser.js` tiene el
mismo patrón en 2 casos (líneas ~71 y ~181). El runner hace `await suite.pruebas(...)` y
lee los contadores inmediatamente: esas 10 comprobaciones se ejecutan "flotando" y NO se
cuentan — un fallo ahí jamás pondría el banco en rojo. Y justo ahí viven los "tests" de
las dos escrituras clínicas (`apiOrdenamientoGuardar`, `apiAccesoAsignarTurno`).

Qué hacer (en este orden, y OJO: el paso 1 solo va a DESTAPAR fallos, no a crearlos):
1. Declara `async pruebas(...)` en ambas suites y antepon `await` a cada `t.casoAsync`,
   como ya hacen suite_15 y suite_21.
2. Corre el runner: los casos que salgan en rojo están PODRIDOS (mocks que ya no calzan
   con el código actual). Repáralos de verdad: el patrón correcto para simular la red es
   inyectar el mock AL CARGAR — `cargar({ fetch: async (url) => ({ok:true, status:200,
   json: async () => ({...})}), gmxhr: (o) => o.onload({status:200, responseText:"..."}) })`
   — porque el userscript captura el fetch original en el arranque (FETCH0) y un
   `win.fetch = ...` posterior llega tarde. Mira los mocks de los casos nuevos de
   suite_15 ("confirmar cita v12.4") como referencia viva.
3. Refuerzo en `tests/runner.js` (o en crearT): contador de casoAsync lanzados vs
   resueltos por suite; si al terminar difieren, falla con "N casoAsync sin await en
   <suite>". Eso convierte este patrón en error imposible de repetir.

Criterio de aceptación: salida completa del runner en el PR con TODAS las comprobaciones
contadas (deberían aparecer ~10 más que antes), cero fantasmas (demuéstralo con el
contador nuevo), y el banco en verde de verdad.

---

## TAREA F — Actualizar LEEME_PRUEBAS.md e INFORME_MUTACIONES.md a la realidad actual

`tests/LEEME_PRUEBAS.md` dice "6 suites" y "alcanzables 239 de 273" — hoy hay 23 suites
y ~283/308 cubiertas; `tests/INFORME_MUTACIONES.md` referencia un banco de 457
comprobaciones (hoy 533+). Actualiza AMBOS documentos con las cifras y la lista de
suites reales (nombre + una línea por suite, sácala de los `nombre:` de cada archivo),
sin cambiar ninguna regla de fondo. No toques ningún .js.

Criterio de aceptación: cifras exactas tomadas de una corrida real del runner pegada en
el PR; cero cambios fuera de esos dos .md.
