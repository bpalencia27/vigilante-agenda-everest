...
function cargar(opciones) {
  ...
  const exportar = "\n;globalThis.__VGL__ = {};\n" +
    ...
    // v17.6.0 — Tres cachés (resumen clínico, medicamentos, tabla oficial) solo
    // caducan comparando Date.now() contra un `ts` guardado; no hay reloj
    // inyectable, así que las pruebas no podían nunca cruzar su TTL sin dormir
    // minutos de verdad. Estos tres ganchos SOLO existen aquí (el cargador de
    // pruebas, que ya declara arriba que no toca el archivo de producción):
    // adelantan el `ts` guardado hacia atrás para simular que pasó tiempo, sin
    // tocar ninguna función real ni su contrato. Cierran sobre la VARIABLE, no
    // sobre un valor: siguen apuntando al objeto vigente aunque guardar()
    // vuelva a reasignarlo.
