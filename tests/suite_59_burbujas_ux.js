...
module.exports = {
  ...
  async pruebas(t, api, env, cargar) {
    ...
    // v18.1.0 — B3.3 (cont.): esta suite también ejercita el `api`/`env` EXTERNO que el
    // runner crea ANTES de este reenvoltorio (los casos de mtrAbrirPanelRedaccion usan
    // ese api directo). Ese contexto necesita el mismo padrón e identidad, o el guard de
    // apertura corta el panel en seco. Un caso que luego cambie SU identidad pisa esto.
    if (env && env.storage && !env.storage.getItem("vgl_acceso_lista")) {
      env.storage.setItem("vgl_acceso_lista", JSON.stringify({
        version: "test-59.acceso",
        perfiles: {
          COMPLETO: [
            { uid: 707, nombre: "Brandon Jesús Palencia Martínez" },
    ...
    if (api && api.__state && !api.__state.activeDoctor.id) {
      api.__state.activeDoctor = { id: 707, name: "BRANDON JESUS PALENCIA MARTINEZ" };
    }
