...
module.exports = {
  ...
  async pruebas(t, api, env, cargar) {
    ...
    t.caso("v16.5.0 — el rediseño del modal, contrato por contrato (entrevista del 20-ago)", () => {
      ...
      // 5. Los modos retirados MAPEAN en vez de reventar (llamadores rezagados).
      t.igual(api.MTR_IA_MODOS_LEGADO ? api.MTR_IA_MODOS_LEGADO.nota_clinica : "analisis_plan", "analisis_plan", "nota_clinica cae en análisis y plan");
    });
    ...
    t.caso("mtrPartirNota separa enfermedad actual y análisis, y nunca pierde texto", () => {
      const dos = api.mtrPartirNota("[ENFERMEDAD ACTUAL]\nEvoluciona estable.\n[ANALISIS Y PLAN]\nContinuar manejo.");
      t.cierto(/estable/.test(dos.enfermedadActual) && /Continuar/.test(dos.analisis), "dos secciones");
      const sinMarcas = api.mtrPartirNota("Un texto plano sin marcadores.");
      t.igual(sinMarcas.enfermedadActual, "", "sin marcas: EA vacío");
      t.cierto(/Un texto plano/.test(sinMarcas.analisis), "y el texto entero como análisis (no se pierde)");
      // Anclaje a inicio de línea: "ANÁLISIS" en medio de la prosa NO parte la nota.
      const enProsa = api.mtrPartirNota("Paciente con dolor. SE PIDE ANALISIS DE ORINA hoy.");
      t.igual(enProsa.enfermedadActual, "", "un 'ANÁLISIS' a mitad de frase no es un marcador");
      t.cierto(/SE PIDE ANALISIS DE ORINA/.test(enProsa.analisis), "la frase queda íntegra");
    });
    ...
    t.caso("mtrJsonV68DesdeResumen mapea lo determinista y deja la prosa en blanco", () => {
      ...
      t.igual(j.ftl_date, "2026-09-01", "FTL");
