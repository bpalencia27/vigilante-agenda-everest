...
//  querySelector por toda la producción, así que esta suite instala en el
//  document del arnés el enriquecedor DOM COMPARTIDO del propio arnés
//  (tests/harness.js: parser de HTML, motor de selectores, dataset y
//  classList con semántica real) antes de abrir el modal. El enriquecedor
//  MUTA los nodos del arnés: nunca toca el archivo de producción.
// =====================================================================
...
module.exports = {
  ...
  async pruebas(t, api, env, cargar) {
    ...
    // ------------------------------------------------------------------
    //  ENRIQUECEDOR DOM — ahora compartido desde el arnés (SF-01)
    //  El parser de HTML, el motor de selectores, dataset, classList y
    //  el disparador de eventos viven en tests/harness.js como única
    //  fuente de verdad para TODAS las suites de simulación. Esta suite
    //  los consume tal cual: su comportamiento no cambia.
    // ------------------------------------------------------------------
    const { instalarDomEnriquecido, disparar: dispararComun } = require("./harness.js");
    const disparar = (nodo, tipo) => dispararComun(nodo, tipo, "[suite_73]");
    ...
    const montar = (cfg) => {
      ...
      // El enriquecedor compartido del arnés parchea createElement y
      // createTextNode: todo nodo que la producción cree a partir de
      // aquí tiene parser, selectores, dataset y classList reales.
