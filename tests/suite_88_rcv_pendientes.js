// =====================================================================
//  SUITE 88 — v18.4.2 / v18.14.2: sección «Próximos exámenes RCV» del
//  módulo «Pendientes» (antes, panel flotante propio)
//
//  Encargo del equipo de riesgo cardiovascular (06-sep-2026) y encargo del
//  10-sep-2026 («los próximos exámenes a ordenar deben dejar de mostrarse de
//  forma desestructurada y agruparse en una sección exclusiva dentro de uno de
//  los módulos existentes del widget lateral»). Esta suite protege, en una
//  frase: que la sección SOLO se arme dentro de la historia clínica de un
//  paciente abierto y SOLO cuando accesoCap("rcv") lo conceda
...
//
//  v18.14.2 — EL PANEL FLOTANTE SE RETIRÓ: el resultado del tick vive en
//  `_rcvUltimo` y lo consume `_pendientesUniversales(doc).rcv`, que es la MISMA
//  vara que alimenta el aviso central de entrada y la pastilla «🩺 Pendientes»
//  del dock. Por eso esta suite lee ese contrato (`rcv.html`, `rcv.n`) en vez
//  de un nodo del DOM: ya no hay nodo propio que leer.
//
...
module.exports = {
  nombre: "Sección Próximos exámenes RCV (v18.4.2/v18.14.2): permiso, contexto HC, fechas y refresco",

  cubre: ["rcvPendientesCalcular", "rcvPendientesDebeVerse", "rcvPendientesFilasHtml",
    "rcvPendientesRotuloPrograma", "rcvPendientesTick", "_ordenesVigentesEstampa",
    "_pendientesUniversales"],
  ...
  async pruebas(t, api, env, cargar) {
    ...
    t.caso("html: chip de estado, par Última→Vence en cada fila, orden por urgencia y escape", () => {
      ...
      const html = api.rcvPendientesFilasHtml(r, "Hipertensión arterial");
      // v18.14.2 — el rótulo de la sección («Próximos exámenes · Riesgo cardiovascular»)
      // vive en avisoUniversal, dueño del cuadro donde la sección se incrusta; aquí solo
      // viajan las filas, para que no haya dos formas de contar los mismos exámenes.
      t.falso(html.indexOf("Próximos exámenes · Riesgo cardiovascular") >= 0, "el constructor de filas no trae el rótulo de la sección");
      t.cierto(html.indexOf("vgl-rcvp-fila") >= 0, "sí trae las filas");
      ...
      const d4 = api.rcvPendientesFilasHtml({ filas: [], nPendientes: 0, sinDatosOrdenes: true, vigenciaDias: null }, "");
    ...
    t.caso("html (v18.8.3): rótulo amable arriba y desc técnica del CUPS debajo — legible sin jerga, sin perder la fuente de verdad", () => {
      ...
      const html = api.rcvPendientesFilasHtml(r, "HTA", "");
      ...
      const h2 = api.rcvPendientesFilasHtml(r2, "", "");
    ...
    t.caso("pie (v18.8.3): con estampa anuncia la hora de lectura; sin estampa, honesto «se actualiza solo»", () => {
      ...
      const conEstampa = api.rcvPendientesFilasHtml(r, "", "hoy 08:15");
      ...
      const sinEstampa = api.rcvPendientesFilasHtml(r, "", "");
      ...
      const ayer = api.rcvPendientesFilasHtml(r, "", "05-09 14:30");
    ...
    // v18.14.2 — el panel flotante se retiró: ya no hay nodo propio que leer. El contrato
    // observable es el que consume el módulo «Pendientes»: _pendientesUniversales(doc).rcv
    // (la MISMA vara que alimenta el aviso central de entrada y la pastilla del dock).
    const rcvDe = (c, cedula) => {
      try { const p = c.api._pendientesUniversales(cedula || "5150076"); return (p && p.rcv) || null; } catch (e) { return null; }
    };
    const montado88 = (c, cedula) => !!rcvDe(c, cedula);

    await t.casoAsync("permiso: el equipo RCV (COMPLETO del padrón) ve la sección dentro de la historia", async () => {
      ...
      const rcv = rcvDe(c, "5150076");
      t.cierto(!!rcv, "la sección llega al módulo «Pendientes»");
      t.cierto(rcv.html.indexOf("Programa: Hipertensión arterial") >= 0, "alineado al programa del paciente");
      t.cierto(rcv.html.indexOf("Última: " + desdeHoy(-200)) >= 0, "muestra la última fecha real de la orden");
      t.cierto(rcv.html.indexOf("Vence: " + desdeHoy(-20)) >= 0, "y su vencimiento");
      t.cierto(rcv.n > 0, "con exámenes por asignar del paquete I10X real");
    });

    await t.casoAsync("permiso (v18.8.1 fail-open): LABORATORIOS y BLOQUEADO NO ven la sección; fuera del padrón y sin identidad SÍ la ven", async () => {
      ...
      // (201: «rcv» no está en su lista de capacidades). La compuerta real de la sección
      ...
      // arma hoy: se aserta el comportamiento real del guard, no el del contrato viejo.
      for (const [uid, nombre] of [[201, "Maryuris Terán"], [999, "Prueba Bloqueada Uno"]]) {
        ...
        await c.api.rcvPendientesTick();
        t.falso(montado88(c), "no se arma para uid " + uid);
      }
      ...
      t.cierto(montado88(fueraPadron.c), "la sección SÍ se arma para el médico fuera del padrón (fail-open)");
      const anon = ctx88(0, "");
      ...
      t.cierto(montado88(anon.c), "y la sección se arma (la compuerta real no exige identidad de médico)");
    });

    await t.casoAsync("v18.8.1 revocación granular: COMPLETO con «rcv» en off (vgl_permisos_locales) no ve la sección; al quitar la entrada vuelve a verla", async () => {
      ...
      t.falso(c.api.esMedicoRCVActivo(), "precondición: con la cap revocada la compuerta de la sección cierra");
      await c.api.rcvPendientesTick();
      t.falso(montado88(c), "COMPLETO con «rcv» revocada NO ve la sección");
      ...
      t.cierto(c.api.esMedicoRCVActivo(), "y la compuerta de la sección abre otra vez");
      await c.api.rcvPendientesTick();
      t.cierto(montado88(c), "al quitar la revocación la sección vuelve a armarse");
      const rcv = rcvDe(c, "5150076");
      t.cierto(!!rcv && rcv.html.indexOf("Programa: Hipertensión arterial") >= 0, "y muestra el contenido real del paciente");
    });
    ...
    await t.casoAsync("D4 en vivo: la red caída no oculta pendientes — todo PENDIENTE con la nota honesta", async () => {
      ...
      await c.api.rcvPendientesTick();
      const rcv = rcvDe(c, "5150076");
      t.cierto(!!rcv, "la sección sigue armada");
      t.cierto(rcv.html.indexOf("Sin órdenes vigentes consultables") >= 0, "la nota lo explica");
      t.falso(rcv.html.indexOf("AL DÍA") >= 0, "nada se da por cubierto");
      t.cierto(rcv.html.indexOf("se actualiza solo") >= 0, "y el pie no finge una hora de lectura (sin consulta exitosa)");
      t.falso(rcv.html.indexOf("leído de Everest") >= 0, "nada de «leído de Everest» con la red caída");
    });
    ...
    await t.casoAsync("refresco en vivo: una orden NUEVA cambia la fila sin recargar nada", async () => {
      ...
      await c.api.rcvPendientesTick();
      t.cierto(rcvDe(c, "5150076").html.indexOf("vgl-rcvp-chip-vencido") >= 0, "precondición: había un examen vencido");
      ...
      await c.api.rcvPendientesTick();
      const rcv = rcvDe(c, "5150076");
      t.falso(rcv.html.indexOf("vgl-rcvp-chip-vencido") >= 0, "ya no está vencido tras la orden nueva");
      t.cierto(rcv.html.indexOf("Vence: " + desdeHoy(175)) >= 0, "el vencimiento recalculado aparece");
    });
    ...
    await t.casoAsync("v18.8.3 frescura: sin consulta exitosa la estampa está vacía; tras el tick dice «hoy», y al día siguiente anuncia el dato de ayer", async () => {
      ...
      const w1 = rcvDe(c, "5150076");
      t.cierto(w1.html.indexOf("leído de Everest hoy 00:00") >= 0, "el pie le dice al médico de CUÁNDO es el dato");
    ...
    await t.casoAsync("sin parpadeo: la firma evita rehacer la sección cuando nada cambió, y cambia de paciente limpia el estado", async () => {
      const { c } = ctx88(102, "Eliseth Estrada");
      await c.api.rcvPendientesTick();
      const r1 = rcvDe(c, "5150076");
      await c.api.rcvPendientesTick();
      t.cierto(rcvDe(c, "5150076") === r1, "el segundo tick con los mismos datos no rehace la sección (firma intacta)");
      // Otro paciente: la sección del anterior no puede quedarse
      cablearHistoria(c, "10101010", true);
      try { c.api.mtrCacheResumenGuardar("10101010", resumen88(888)); } catch (e) {}
      await c.api.rcvPendientesTick();
      t.cierto(rcvDe(c, "10101010") !== r1, "al cambiar de paciente el contenido se renueva");
      t.cierto(!!rcvDe(c, "10101010"), "y queda disponible para el nuevo paciente");
    });

    await t.casoAsync("anti-cruce: si el paciente cambió mientras salía la consulta, no se atribuye nada del anterior", async () => {
      ...
      await p;
      t.falso(!!rcvDe(c, "20202020"), "nada del paciente anterior se le atribuye al nuevo");
      t.falso(!!rcvDe(c, "5150076"), "y el estado del anterior quedó limpio tras el cruce");
    });

    // ============ NAVEGACIÓN DINÁMICA: enganche en tick() y retiro ============
    await t.casoAsync("enganche real: tick() (la ruta de producción) arma la sección sin llamarlo a mano", async () => {
      const { c } = ctx88(104, "Sinaí Mijares");
      try { c.api.__state.killed = false; } catch (e) {}
      c.api.tick();
      await new Promise((res) => setTimeout(res, 40));
      t.cierto(montado88(c), "el tick de la agenda armó la sección (el hook _rumTramo vive)");
    });

    await t.casoAsync("navegación dinámica: al volver a Citas del día, tick() RETIRA la sección", async () => {
      const { c } = ctx88(104, "Sinaí Mijares");
      await c.api.rcvPendientesTick();
      t.cierto(montado88(c), "precondición: armada en la historia");
      ...
      c.api.tick();
      t.falso(montado88(c), "tick() retiró la sección al salir de la historia (no queda colgando sobre Citas del día)");
    });

    // ============================ CSS / REGISTROS ============================
    t.caso("fuente (F1, v18.14.2): la sección vive DENTRO del módulo «Pendientes» y el panel flotante ya no existe", () => {
      const code = fs.readFileSync(path.join(__dirname, "..", "vigilante_agenda.user.js"), "utf8").replace(/\r\n/g, "\n");
      // (a) la sección se inyecta en el cuadro del módulo, con el mismo lenguaje visual que PyM
      t.cierto(code.indexOf('Próximos exámenes · Riesgo cardiovascular (\' + (rcv.n || 0) + \' por asignar)') >= 0,
        "el rótulo de la sección se pinta dentro del cuadro de Pendientes");
      t.cierto(code.indexOf('if (rcv && rcv.html) {') >= 0, "la sección solo existe si hay html calculado por el tick");
      // (b) el dock pasa el dato por la MISMA vara (_pendientesUniversales), no por una segunda
      t.cierto(code.indexOf("anexo5: _p.anexo5, rcv: _p.rcv") >= 0,
        "la pastilla 🩺 Pendientes reabre el cuadro con la sección RCV incluida");
      // (c) el panel flotante retirado NO dejó rastros: ni id, ni botones, ni pastilla
      for (const muerto of ["#vgl-rcv-pendientes", "#vgl-rcv-pendientes-pill", ".vgl-rcvp-cerrar", ".vgl-rcvp-min", "vgl_rcvp_pos"]) {
        t.falso(code.indexOf(muerto) >= 0, "sin rastro de " + muerto);
      }
    });

    t.caso("CSS: la sección está registrada en tokens oscuro+claro y sus colores llevan !important", () => {
      const code = fs.readFileSync(path.join(__dirname, "..", "vigilante_agenda.user.js"), "utf8").replace(/\r\n/g, "\n");
      ...
      const i0 = css.indexOf("\n      #vgl-pym-modal .vgl-rcvp-prog{");
      t.cierto(i0 >= 0, "existe la primera regla de la sección (anclada a línea propia, no a la lista de tokens)");
      const iFin = css.indexOf("/* v17.32.0/v17.41.0", i0);
      const bloque = css.slice(i0, iFin > i0 ? iFin : i0 + 4000);
      ...
      t.falso(/animation/.test(bloque), "la sección no anima nada");
      // (c) el cuadro que la hospeda está en las DOS listas de tokens (oscura y clara) —
      // sin la clara, el tema claro hereda Everest
      t.cierto(/#vgl-chooser-modal,#vgl-a5-modal\{/.test(css), "el cuadro está en la lista de tokens oscura");
      t.cierto(/#vgl-chooser-modal\.light,#vgl-a5-modal\.light\{/.test(css), "y en la clara");
      // (d) modo oculto: el cuadro de Pendientes ya está cubierto por #vgl-pym-modal
      t.cierto(css.indexOf("body.vgl-modo-oculto #vgl-pym-modal") >= 0, "el modo oculto esconde el cuadro que la hospeda");
      // (e) cada var() que consume el bloque está declarada en la lista de tokens
      const iDark = css.indexOf("#vgl-paquete-modal,#vgl-chooser-modal,#vgl-a5-modal{");
      ...
      // (f) NINGUNA regla de .vgl-rcvp-* quedó huérfana. Es el riesgo exacto de este
      // cambio: al retirar el panel flotante, un selector que siga apuntando a
      // #vgl-rcv-pendientes ya no casa con nada, la fila se queda sin regla y el color
      // se hereda del ancestro — que en este proyecto casi siempre es el azul de Everest.
      // Todas tienen que colgar del cuadro que las hospeda (#vgl-pym-modal).
      const huerfanas = [];
      const cssSinComentarios = css.replace(/\/\*[\s\S]*?\*\//g, "");   // los comentarios no son selectores
      for (const m of cssSinComentarios.match(/[^{}]+\{[^}]*\}/g) || []) {
        const sel = m.split("{")[0];
        if (sel.indexOf(".vgl-rcvp-") < 0) continue;
        for (const alt of sel.split(",")) {
          if (alt.indexOf("#vgl-pym-modal") < 0) huerfanas.push(alt.trim());
        }
      }
      t.igual(huerfanas.join(" | "), "", "toda regla de .vgl-rcvp-* cuelga de #vgl-pym-modal (sin selector huérfano, sin color heredado de Everest)");
    });
  },
};
