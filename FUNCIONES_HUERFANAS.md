...
- **Base de escaneo:** Rama `claude/v14-continuacion` / `feat/motor-portado`.
...
| Función | Línea (aprox) | Categoría | Qué creo que iba a hacer (Análisis) |
| :--- | :--- | :--- | :--- |
...
| `vigenciaPorEstadio` | 3340 | Enganchada (T3) | Calcula la vigencia en días permitida para un analito específico basándose en el estadio (ej. ERC), verificando precondiciones como el programa o si el paciente padece de DM2. *(Enganchada en T3 a `_analitosRcvVencidos`)*. |
| `analitoTablaDesdeClaveRcv` | 3370 | Enganchada (T3) | Sirve como un mapa de traducción seguro para convertir las claves nativas del sistema RCV (`CREATININA`) a las llaves semánticas de la tabla local en el script (`creatinina`). *(Enganchada en T3)*. |
...
| `_demograficosInvalidar` | 10459 | Deuda muerta | Iba a limpiar la caché interna demográfica del paciente para obligar a un re-fetch. Su definición no tiene ningún llamador en el código. |
